...
clean_column <- function(column) {
  ...
  # Remove actual NA values, 'NA' strings, and any variations (e.g., ' na', 'Na', etc.)
...
# Step 2: Combine the cleaned values into a single list or vector
...
# Step 3: Remove duplicates and sort the final list of choices
...
server <- function(input, output, session) {
  ...
  # Update the available Visualisations based on selected patient
  observeEvent(input$patient_names, {
    ...
    # Add 'All Plots' as the first choice
    updateSelectInput(session, "visualisation_selection", choices = c("All Plots", available_Visualisations))
  })
  
  # Render the patient report
  output$patient_report <- renderUI({
    req(input$patient_names)
    selected_patient_data <- Pt_Wide_reactive()
    patient_info <- selected_patient_data[1, c("Patient_Name", "Sex", "Diagnoses_1", "Prescription_1")]
    
    tags$div(
      tags$b("Patient Name: "), patient_info$Patient_Name, tags$br(),
      tags$b("Sex: "), patient_info$Sex, tags$br(),
      tags$b("Diagnoses: "), patient_info$Diagnoses_1, tags$br(),
      tags$b("Prescription(s): "), patient_info$Prescription_1, tags$br()
    )
  })

  # Render the combined plot based on selected visualisation
  output$combined_plot <- renderPlotly({
    ...
    # Filter data based on the selected Visualisation
...
output$patient_report <- renderUI({
  ...
  # Convert Patient_DOB to Date format with 'dd-mm-yyyy'
  if (!is.null(filtered_data$Patient_DOB[1]) && !is.na(filtered_data$Patient_DOB[1])) {
    patient_dob <- as.Date(filtered_data$Patient_DOB[1], format = "%d-%m-%Y")  # Adjusted format
  } else {
    patient_dob <- NA
  }
  ...
  if (!is.na(patient_dob)) {
    patient_age <- floor(as.numeric(difftime(Sys.Date(), patient_dob, units = "weeks")) / 52.25)  # Approximate age in years
  } else {
    patient_age <- "Unknown"
  }
  ...
  ## Creating the final report with DOB and Age
