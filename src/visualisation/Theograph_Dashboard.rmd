...
patient_ids <- unique(dummy_TS_df$Patient)
```
...
selectInput("patient_id", "Select Patient ID:", choices = patient_ids)
```
...
# Load the dummy data
timeseries_path <- "C:/Users/elliot.royle/OneDrive - Midlands and Lancashire CSU/Git/Primary-Care-Theograph/data/processed_extracts/DUMMY_RStudio_Data.xlsx"
dummy_TS_df <- readxl::read_excel(timeseries_path, sheet = 1)
dummy_TS_df$Date <- as.Date(dummy_TS_df$Date)
patient_ids <- unique(dummy_TS_df$Patient)
...
# Shiny app UI (Removed duplicated dropdown here)
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      # No need for selectInput() here as it's now in the sidebar
    ),
    mainPanel(
...
server <- function(input, output) {
  ...
  # Reactive expression to filter data based on the selected patient
  TS_df <- reactive({
    req(input$patient_id)  # Ensure a patient is selected
    filtered_data <- dummy_TS_df %>%
      filter(Patient == input$patient_id)
    
    print(filtered_data)  # Check if data is filtered correctly
    filtered_data  # Return filtered data
  })
  
  # Render the combined plot using Plotly
  output$combined_plot <- renderPlotly({
    filtered_data <- TS_df()  # Use the reactive filtered data
    
    if (nrow(filtered_data) == 0) {
      return(NULL)  # Return if no data available
    }
    
    plot_1 <- create_time_series_plot(filtered_data, "Date", "Biomedical_Value_1", "Biomedical_Test_1", "Biomedical_1_Units", "blue")
    plot_2 <- create_time_series_plot(filtered_data, "Date", "Drug_1_Value", "Drug_Name_1", "Drug_1_Units", "lightblue")
    plot_3 <- create_time_series_plot(filtered_data, "Date", "Biomedical_Value_2", "Biomedical_Test_2", "Biomedical_2_Units", "orange")
    plot_4 <- create_time_series_plot(filtered_data, "Date", "Drug_2_Value", "Drug_Name_2", "Drug_2_Units", "yellow")
