...
create_time_series_plot <- function(data, date_col, y_col, test_col, unit_col, vis_colour, episodicity_col) {
  tryCatch({
    ...
    data$tooltip_text <- paste0(
      "Date: ", format(data[[date_col]], "%d/%m/%Y"), "<br>",  # Use <br> for line breaks in HTML
...
# Ensure the tooltip is included correctly in the ggplotly call
plotly_plot <- ggplotly(plot, tooltip = "text") %>%  # Specify the text aesthetic
...
create_diagnosis_plot <- function(data, date_col, diagnosis_col, vis_colour) {
  tryCatch({
    ...
    # Create the plot
    plot <- ggplot(data, aes(x = !!sym(date_col), y = 1)) +  # Use a constant value for Y-axis
      geom_line(color = vis_colour) +
      geom_point(aes(text = tooltip_text), color = vis_colour, size = 2) +  
      labs(title = NULL, x = "Date", y = NULL) +  # No Y-axis label
      scale_x_date(date_labels = "%d/%m/%Y", limits = c(as.Date("2020-01-01"), as.Date(Sys.Date()))) +
      theme_minimal() +
      theme(axis.text.y = element_blank(), 
            axis.ticks.y = element_blank(), 
            axis.title.y = element_blank())   # Removes minor y-axis gridlines (if any))  # Remove Y-axis scale
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    plotlyOutput("combined_plot", height = "800px", width = "100%"),  # Wrapped in div for styling
    )
  )
...
# Create the plots
plot_1 <- create_time_series_plot(filtered_data, "Contact_Event_Date", "Biomedical_Value_1", "Biomedical_Test_1", "Biomedical_1_Units", "#407EC9")
plot_2 <- create_time_series_plot(filtered_data, "Contact_Event_Date", "Drug_1_Value", "Drug_Name_1", "Drug_1_Units", "#ff9933")
plot_3 <- create_time_series_plot(filtered_data, "Contact_Event_Date", "Biomedical_Value_2", "Biomedical_Test_2", "Biomedical_2_Units", "#407EC9")
plot_4 <- create_time_series_plot(filtered_data, "Contact_Event_Date", "Drug_2_Value", "Drug_Name_2", "Drug_2_Units", "#ff9933")

diagnosis_plot_1 <- create_diagnosis_plot(filtered_data, "Contact_Event_Date", "Diagnosis_1", "#fa5c5c")
diagnosis_plot_2 <- create_diagnosis_plot(filtered_data, "Contact_Event_Date", "Diagnosis_2", "#36a358")

# Swap plot_2 and plot_3 in the plot_list
plot_list <- list(diagnosis_plot_1, diagnosis_plot_2, plot_1, plot_3, plot_2, plot_4)

# Adjust the label_list accordingly
label_list <- list(
...
# If no valid plots, show a notification and return NULL
if (length(valid_plots) == 0) {
  showNotification("No valid plots available for the selected patient.", type = "warning")
  return(NULL)
}

# Create dynamic Y-axis labels with line breaks
combined_plot <- subplot(valid_plots, 
  ...
  layout(
    ...
    yaxis = list(showticklabels = FALSE, automargin = TRUE),
    
    # Add annotations dynamically for valid labels
    annotations = lapply(seq_along(valid_labels), function(i) {
      list(
        x = -0.10, 
        y = 1 - (i - 1) * (1 / length(valid_labels)) - 0.11,  # Dynamic Y-positioning and added spacing adjustment
        text = valid_labels[[i]], 
        showarrow = FALSE, 
        xref = "paper", 
        yref = "paper", 
...
# Run the app
shinyApp(ui = ui, server = server, options = list(height = 1250))
```